...
| <a name="input_hostname"></a> [hostname](#input\_hostname) | Hostname override. When set, this takes precedence over hostname\_template. | `string` | `""` | no |
| <a name="input_hostname_template"></a> [hostname\_template](#input\_hostname\_template) | The `format()` string to use to generate the hostname via `format(var.hostname_template, var.tenant, var.stage, var.environment)`"<br/>Typically something like `"echo.%[3]v.%[2]v.example.com"`. | `string` | `""` | no |
